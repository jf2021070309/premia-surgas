                ...
                <?php if (!empty($historial)): ?>
                    ...
                    <?php 
                        ...
                        foreach ($historial as $v) {
                            $fechaStr = date('d M, Y', strtotime($v['fecha']));
                            $grupos[$fechaStr][] = $v;
                        }
                    ?>
                            ...
                            <?php foreach ($items as $v): ?>
                                <div class="history-card">
                                    ...
                                    <div class="col-detail">
                                        <?php 
                                            // Limpiar retornos de carro (Windows) y caracteres invisibles
                                            $detalleRaw = str_replace(["\r", "), ", ", Recarga"], ["", ")\n• Recarga", "\n• Recarga"], $v['detalle']);
                                            $lineas = explode("\n", $detalleRaw);
                                            
                                            foreach ($lineas as $linea) {
                                                // Ignorar líneas que no contengan al menos una letra o número (elimina rayas y basura unicode)
                                                if (!preg_match('/[a-zA-Z0-9]/', $linea)) {
                                                    continue;
                                                }
                                                
                                                // Quitar espacios, guiones manuales o balas literales viejas
